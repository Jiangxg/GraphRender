...
// TODO: 适配dev_input_target_camera_in_inv
// TODO: 适配dev_input_target_camera_ex：[R|t]
__global__ void aabb_intersect_point_kernel(
  ...
  // global to shared memory: dev_input_patches_no
  
  // RTX3090的共享内存只有100k， 但实际能用的远远不到这个数
  // 循环使用，每次遍历完之后再加入新的patch的信息
  __shared__ unsigned char dev_input_patches_no_shared[2 * 16 * 33 * 40];

  int t = threadIdx.x;
  
  int patchesEachThread = (16 * 33 * 40 + THREADNUM - 1) / THREADNUM;
  //printf("patchesEachThreads: %d\n", patchesEachThread);

  for (int i = t * patchesEachThread; i < (t + 1) * patchesEachThread; i++) {
    if (i >= 16 * 33 * 40) break;
    dev_input_patches_no_shared[2 * i] = static_cast<unsigned char>(dev_input_patches_no[3 * i]);
    dev_input_patches_no_shared[2 * i + 1] = static_cast<unsigned char>(dev_input_patches_no[3 * i + 1]);
  }
  __syncthreads();
  ...
  //printf("dev_input_target_camera_in_inv[0] %.4f \n", dev_input_target_camera_in_inv[0]);
  //printf("dev_input_target_camera_ex[0] %.4f \n", dev_input_target_camera_ex[0]);
  ...
  //float wpointHomo = dev_input_target_camera_ex[12] * pointX + dev_input_target_camera_ex[13] * pointY + dev_input_target_camera_ex[14] * pointZ + dev_input_target_camera_ex[15];
  
  //wpointX = wpointX / wpointHomo;
  //wpointY = wpointY / wpointHomo;
  //wpointZ = wpointZ / wpointHomo;
  
  
  //printf("wpointX: %.4f, wpointY %.4f, wpointZ %.4f, originX %.4f, originZ %.4f, originZ %.4f from Idx %d \n", wpointX, wpointY, wpointZ, originX, originY, originZ, Idx);
  ...
  int Bingo = 0;
  int Oops = 0;
  // 经过测试，只是遍历的话，只需要574微秒！！（32x33x40个平面）
  for (int k = 0, cnt = 0; k <  patch_num && cnt < n_max; ++k) {
    
    // 读取太慢了！！！，每一次都要读取，太慢了
    // 如果能够跳跃式的读取就好了
    // 把patches
    // for循环中光是跑这个if判断语句，时间都能飙升至80ms!!???  
    //if (!hit[(int)dev_input_patches_no_shared[2 * k] - 1][(int)dev_input_patches_no_shared[2 * k + 1] - 1]) continue;
    //if (!hit[(int)dev_input_patches_no[3 * k] - 1][(int)dev_input_patches_no[3 * k + 1] - 1]) continue;
    
    if (k < 16 * 33 * 40) {
      if (!hit[(int)dev_input_patches_no_shared[2 * k] - 1][(int)dev_input_patches_no_shared[2 * k + 1] - 1]) {
        Oops++;
        continue;
      }
    } 
    else {
      if (!hit[(int)dev_input_patches_no[3 * k] - 1][(int)dev_input_patches_no[3 * k + 1] - 1]) {
        Oops++;
        continue;
      }
    }
  ...
  /*
  dev_output[Idx * 3] = static_cast<unsigned char>(planeIdx * 8);
  dev_output[Idx * 3 + 1] = static_cast<unsigned char>(planeIdx * 8);
  dev_output[Idx * 3 + 2] = static_cast<unsigned char>(planeIdx * 8);
  */
}
